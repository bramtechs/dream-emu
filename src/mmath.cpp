...
template <typename T>
T Lerp(T start, T end, T amount)
{
    T result = start + amount*(end - start);
    return result;
}
...
{
    T result = (value - inputStart)/(inputEnd - inputStart)*(outputEnd - outputStart) + outputStart;
    return result;
}
...
{
    T result = value - (max - min)*floorf((value - min)/(max - min));
    return result;
}
